...
fn create_fat_filesystem(fat_path: &Path, efi_file: &Path, elf_file: &Path) {
    ...
    io::copy(&mut fs::File::open(&elf_file).unwrap(), &mut kernel).unwrap();
}
...
fn main() {
    ...
    let a1: Vec<&str> = args[1].split(",").collect();
    println!("a1: {:?}", a1);
    ...
    create_fat_filesystem(&fat_path, &efi_path, &elf_path);
