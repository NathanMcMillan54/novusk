...
unsafe extern "C" fn get_irqn() -> i16 {
    ...
    let irq_addr = peripheral_addr + SOC_INFO.get("IRQ Offset");
...
unsafe fn bcm2837_irqchip_setup() -> i16 {
    ...
    if peripheral_address == 0 || irq_offset == 0 {
        return 1;
    }

    let irq_address = peripheral_address + irq_offset;
...
pub mod handlers {
    ...
    use core::ops::Add;
    use bcm_soc::bcm2837::SOC_INFO;
    use bcm_soc::bcm2837::timer::BCM2837_TIMER;
    use novuskinc::timer::Timer;
    ...
    pub unsafe extern "C" fn TIMER1_irq() -> i16 {
        ...
        let peripheral_addr = SOC_INFO.get("Peripheral Address");
        let timer = peripheral_addr + SOC_INFO.get("Timer CS Offset");
