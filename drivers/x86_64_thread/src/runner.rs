// Copied from: Phillip Opperman https://os.phil-opp.com/async-await/
...
use crate::{interrupt_wake, interrupt_wakeups};
            ...
            match task.as_mut().poll(&mut context) {
                ...
                Poll::Pending => {
                    ...
                    if self.pending_tasks.insert(task_id, task).is_some() {
                        unsafe { kerror!("Task with same ID already in pending_tasks"); }
                    }
                }
            }
        }
    ...
    fn wake_waiting_tasks(&mut self) {
        while let Ok(task_id) = self.wake_queue.pop() {
            if let Some(task) = self.pending_tasks.remove(&task_id) {
                ...
                unsafe { kerror!("Woken task not found in pending_tasks"); }
            }
        }
    }
